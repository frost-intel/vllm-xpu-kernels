...
// to a whole 128-wide tile that brings it back to exactly 512. The policy has
// no handle on head_size_vo, so kSplitVExpectedShapeOutV below pins that down.
...
// The only V extent split-V is instantiated for. Asserted in the policy and
// checked against the runtime head_size_vo by the xe_2 dispatcher.
...
struct decode_policy_kv64_splitv {
  static constexpr int kShapeOutV =
      decode_splitv_shapeout_v_value(q_packed::value, head_dim::value);

  using HeadDim = head_dim;
  using ShapeQK = Shape<q_packed, _64, _64>;
  using ShapePV = Shape<q_packed, _128, _64>;
  using ShapeOut = Shape<q_packed, cute::Int<kShapeOutV>>;
  using SubgroupLayoutQK = Layout<Shape<_1, _4, _1>>;
  ...
  // Also catches the floor-to-zero case at large q_packed.
  static_assert(
      kShapeOutV == kSplitVExpectedShapeOutV,
      "split-V derives its V extent from head_dim (the Q/K bucket) and "
      "assumes it lands on the shape's real V width; it no longer does");
};
