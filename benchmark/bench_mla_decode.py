...
one query token per sequence, 1 KV head, head_size_qk 576 (kv_lora_rank 512 +
qk_rope_head_dim 64) and head_size_vo 512, against a paged KV cache.

Sweeps batch size, KV length, per-sequence length spread, block size and the
number of KV splits. Configurations the build does not provide a compiled
policy for are reported as FAIL rather than hidden.

The ``eff%`` column compares against a bandwidth roofline for the KV bytes the
configuration has to read, so it is only meaningful once the KV working set is
larger than L2; below that the cache serves the reads and the column will
exceed what HBM could deliver.

Usage:
    python benchmark/bench_mla_decode.py
    python benchmark/bench_mla_decode.py --batches 1 --splits 0 8 --block-sizes 64
"""
...
# csrc/xpu/attn/fmha_utils.hpp (decode_policy_qpacked_head).
...
def build_inputs(batch, seq_lens, num_heads_q, block_size, dtype, seed=0):
    ...
    # vLLM's XPUMLAImpl.forward_mqa does) gets a uniform split count from
    # get_num_splits() instead of a per-sequence plan.
